...
> Vendored from the upstream [`github/awesome-copilot/skills/audit-integrity`](https://github.com/github/awesome-copilot/tree/main/skills/audit-integrity) skill (MIT). Unchanged from upstream — pure methodology framework (no runtime tool, no external dependency). Pairs well with Lanyard's `review` skill as a post-lens quality gate, and with the lean-ctx self-learning regenerator: this skill's `self-learning-system` reference formalises the lesson/memory pattern that Lanyard's `## Learned patterns` block implements at the workspace level.
