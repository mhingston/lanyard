...
> Vendored from the upstream [`github/awesome-copilot/agents/ai-readiness-reporter`](https://github.com/github/awesome-copilot/blob/main/agents/ai-readiness-reporter.agent.md) custom agent (MIT). Companion to the `acreadiness-assess` skill: invoked by the skill's reporter step to render the `agentrc readiness --json` envelope into `reports/index.html` using the bundled `report-template.html`. Unchanged from upstream.
